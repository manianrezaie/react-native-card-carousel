...
export default class CarouselCard extends Component {
    ...
    refreshState(){    
        ...
        for(let i = 0; i < this.arrLength+4; i++) {
            _scaleYArr.push(new Animated.Value(0));
            _translateYArr.push(new Animated.Value(0));
        }
        this.state = {
            ...this.state,
            scaleYArr: _scaleYArr,
            translateYArr: _translateYArr
        };
        this._cardAnimated(1);
    }
    componentDidMount() {
        setTimeout(() => {
            if(this.mainScroll) {
    ...
    _onAssistScroll(e) {
        if(this.mainScroll && this.assistScroll) {
            ...
            let _currentPageInt =Math.abs(parseInt(this.currentPageFloat) ) +1 ;
             if(_currentPageInt !== this.currentPageInt){ 
                    this.currentPageInt = _currentPageInt;
                
                if(this.props.onPageChanged){
                    this.props.onPageChanged(this.currentPageInt , this.arr[this.currentPageInt]);
                }
            }
    ...
    render() {
        ...
        return (
            <View>
                ...
                <ScrollView
                    style = {{ width: this.moveDistance, height: this.height/4*3, position: 'absolute', left: (this.width-this.moveDistance)/2, top: this.height/4}}
                    horizontal = {true}
                    pagingEnabled = {true}
